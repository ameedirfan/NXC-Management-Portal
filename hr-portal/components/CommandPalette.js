    ...
    >
      ...
      <div className="relative h-fit w-full max-w-lg overflow-hidden rounded-2xl border border-white/50 border-t-white/80 bg-brand-50/70 shadow-2xl backdrop-blur-xl backdrop-saturate-150">
        <div className="flex items-center gap-2 border-b border-brand-200/70 px-4 py-3">
          <Search size={16} className="shrink-0 text-brand-700" aria-hidden="true" />
          <input
            autoFocus
            value={query}
            onChange={(e) => setQuery(e.target.value)}
            placeholder="Jump to a tab…"
            className="min-w-0 flex-1 bg-transparent text-sm text-brand-900 outline-hidden placeholder:text-brand-700"
          />
          <span className="shrink-0 rounded-sm border border-brand-300 px-1.5 py-0.5 text-[10px] font-medium text-brand-700">
            ESC
          </span>
        </div>
        <div className="max-h-72 overflow-y-auto p-2">
          {filtered.length === 0 && <p className="px-3 py-2 text-sm text-brand-700">No matches.</p>}
          {filtered.map((t) => {
